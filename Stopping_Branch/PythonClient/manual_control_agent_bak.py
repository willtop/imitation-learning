...
def make_carla_settings(args):
    ...
    cameraNN.set(FOV=100)
    cameraNN.set_image_size(800, 600)
    cameraNN.set_position(2.0, 0.0, 1.4)
    cameraNN.set_rotation(-15.0, 0, 0)
...
class CarlaGame(object):
    def __init__(self, carla_client, nn_agent, args):
    ...
    def _on_loop(self):
        ...
        if self._countFrames == 0:
            meas_array = np.zeros((200,88,200,3))
            image_array = np.zeros((200,28))

        # Print measurements every second.
        if self._timer.elapsed_seconds_since_lap() > 1.0:
            print("One second has passed")
            image_array[self._countFrames,:,:,:] = image_converter.to_rgb_array(self._h5_image)

            print(image_array)
            with h5py.File('random.h5', 'w') as f:
                dset = f.create_dataset("image_array", (H5_WINDOW_HEIGHT,H5_WINDOW_WIDTH,3))
                dset[:] = image_array
            if self._city_name is not None:
                ...
                print(measurements.player_measurements)
