...
class HumanDriver(object):
    ...
    def computeControl(self):
        ...
        brakeCmd = 1.6 + (2.05 * math.log10(-0.7 * 1 + 1.4) - 1.2)/0.92
        ...
        # if self.jsButtons[self.cmd_left_idx]:
        #     self.cmd = 3
        # elif self.jsButtons[self.cmd_right_idx]:
        #     self.cmd = 4
        # elif self.jsButtons[self.cmd_straight_idx]:
        #     self.cmd = 5
        # else:
        #     self.cmd = 2
    ...
    def _on_loop(self):
        ...
        if self._countFrames == self._sizeBatch:
            ...
            with h5py.File(abs_file_path, 'w') as f:
                ...
                # dset_image = f.create_dataset("rgb", (self._sizeBatch,H5_WINDOW_HEIGHT,H5_WINDOW_WIDTH,3))
                # dset_image = self._image_array
                print("RGB ARRAY AFTER WRITING H5")
                print(self._image_array)
                print("END RGB ARRAY AFTER WRITING H5")

                dset_meas = f.create_dataset("targets", data=self._meas_array, dtype='f8')
                # dset_meas = f.create_dataset("targets", (self._sizeBatch,self._numTarget))
                # dset_meas = self._meas_array
                print("TARGET ARRAY AFTER WRITING H5")
                print(self._meas_array)
                print("END TARGET ARRAY AFTER WRITING H5")
        ...
        if self._timer.elapsed_seconds_since_lap() > 1.0:
            ...
            # print(image_converter.to_rgb_array(self._h5_image).shape)
            self._image_array[self._countFrames,:,:,:] = image_converter.to_rgb_array(self._h5_image)
        ...
        # if self._countFrames == 200:
        #     self._countFrames = 0
        #     rel_path = "h5/train{}.h5".format(self._globalCount)
        #     abs_file_path = os.path.join(script_dir, rel_path)
        #     with h5py.File(abs_file_path, 'w') as f:
        #         dset_image = f.create_dataset("image_array", (H5_WINDOW_HEIGHT,H5_WINDOW_WIDTH,3))
        #         dset_image[:] = self._image_array
        #         dset_meas = f.create_dataset("meas_array", (H5_WINDOW_HEIGHT,H5_WINDOW_WIDTH,3))
        #         dset_meas[:] = self._meas_array
        #     self._globalCount += 1
